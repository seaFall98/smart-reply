...
8. 首版以 Claude Code `2.1.163` 为最低已测试版本；更早版本不保证兼容。
...
Hook 解析本轮覆盖标记后，使用 `session_id` 将本轮模式写入插件状态目录，
供 `Stop` Hook 读取。状态只保存模式和必要时间戳，不保存用户 Prompt。
...
- 从本轮状态读取到 `inline` 或 `off` 模式时放行；
- 最终回复过长时阻止结束一次；
- 阻止原因要求 Claude 将完整内容写入 Smart Reply 文档，并将最终回复缩短；
- `stop_hook_active` 为 `true` 时放行，避免死循环；
- `last_assistant_message` 缺失或类型错误时放行；
...
首版 Stop Hook 只机械限制最终回复长度，不能证明 Agent 已经创建文档，也不读取
完整 transcript 验证写文件行为。文档生成主要依赖 Skill 和前置行为引导。
...
首版状态主要用于在 `UserPromptSubmit` 与 `Stop` 之间传递本轮覆盖模式。
Stop 重入优先使用 Claude Code 提供的 `stop_hook_active` 字段判断，不自行维护重试计数。
...
该命令已在 Claude Code `2.1.163` 上确认可用。README 必须记录实际测试版本。
...
该命令已在 Claude Code `2.1.163` 上确认可用。若未来 Claude Code 更改插件验证接口，
应更新测试和 README，而不是静默跳过验证。
